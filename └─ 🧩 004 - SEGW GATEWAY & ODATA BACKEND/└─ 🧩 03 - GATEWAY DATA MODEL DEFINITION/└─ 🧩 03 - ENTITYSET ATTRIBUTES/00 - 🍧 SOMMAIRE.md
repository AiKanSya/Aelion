# 🌸 SOMMAIRE — ENTITYSET ATTRIBUTES

## 🌺 OBJECTIFS DU COURS

- [ ] Comprendre la progression générale du cours.
- [ ] Identifier l’ordre conseillé des chapitres.
- [ ] Retrouver rapidement une notion précise.

## 🌺 PARCOURS

```mermaid
flowchart TD
    A["ENTITYSET ATTRIBUTES"]
    A --> B["ENTITYSET - NAME"]
    B --> C["ENTITYSET - ENTITYTYPE"]
    C --> D["ENTITYSET - LABEL"]
    D --> E["ENTITYSET - SEMANTICS (OPTIONAL)"]
    E --> F["ENTITYSET - ACTIONS"]
```

> [!IMPORTANT]
> Suivre les chapitres dans l’ordre numérique, sauf révision ciblée d’une notion déjà étudiée.

## 🌺 CHAPITRES

- [ENTITYSET - NAME](<./01 - 🍧 ENTITYSET - NAME.md>)
- [ENTITYSET - ENTITYTYPE](<./02 - 🍧 ENTITYSET - ENTITYTYPE.md>)
- [ENTITYSET - LABEL](<./03 - 🍧 ENTITYSET - LABEL.md>)
- [ENTITYSET - SEMANTICS (OPTIONAL)](<./04 - 🍧 ENTITYSET - SEMANTICS (OPTIONAL).md>)
- [ENTITYSET - ACTIONS](<./05 - 🍧 ENTITYSET - ACTIONS.md>)

<details>
<summary>Afficher la méthode de travail</summary>

1. Lire les objectifs du chapitre.
2. Étudier le schéma Mermaid.
3. Reproduire les exemples.
4. Réaliser l’exercice sans consulter la correction.
5. Valider l’auto-évaluation.

</details>

## 🌺 RÉSUMÉ

> - Ce sommaire représente le parcours du cours **ENTITYSET ATTRIBUTES**.
> - Les liens pointent vers les chapitres conservés dans leur emplacement d’origine.
